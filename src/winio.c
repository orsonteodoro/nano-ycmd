...
int get_kbinput(WINDOW *frame, bool showcursor) {
	...
	while (kbinput == ERR) {
		...
		if (kbinput == ERR && frame == get_popup()) {
			...
			if (ch == 0x1B) {
				kbinput = 0x1B;
				debug_log("Detected raw ESC (0x1B), frame=%p", (void *)frame);
				timeout(20);
				int trailing;
				while ((trailing = wgetch(frame)) != ERR) {
					debug_log("Consumed trailing byte 0x%x", trailing);
				}
				timeout(50);
				break;
			} else if (ch == KEY_RESIZE) {
				debug_log("Terminal resized, closing popup");
				hide_completions();
				ycmd_handling = FALSE;
				is_popup_mode = FALSE;
				refresh_needed = TRUE;
				...
				ungetch(ch);
				debug_log("Pushed back ch=0x%x", ch);
			}
		}
		clock_gettime(CLOCK_MONOTONIC, &now);
		double elapsed = (now.tv_sec - start.tv_sec) + (now.tv_nsec - start.tv_nsec) / 1e9;
		if (elapsed > 5.0 && frame == get_popup()) {
			debug_log("Popup timeout after %.2fs, closing popup", elapsed);
			hide_completions();
			ycmd_handling = FALSE;
			is_popup_mode = FALSE;
			refresh_needed = TRUE;
	...
	if (frame == get_popup()) {
		...
		if (kbinput == KEY_UP || kbinput == KEY_DOWN) {
			debug_log("Popup navigation input=0x%x", kbinput);
		}
	}
	...
	debug_log("returning 0x%x", kbinput);
	return kbinput;
}
#else
...
void edit_redraw(linestruct *old_current, update_type manner) {
	debug_log("old_current=%p, current=%p, filetop=%p, manner=%d",
	...
	if (current_is_offscreen()) {
		...
		refresh_needed = TRUE;
		debug_log("Current offscreen, setting refresh_needed");
		return;
	}

	if (openfile->filetop == NULL || openfile->current == NULL) {
		debug_log("Invalid linestruct, filetop=%p, current=%p",
	...
	if (!is_popup_active()) {
		...
		debug_log("Cleared midwin (popup inactive)");
	}
	...
	while (line != NULL && row < maxrows) {
		...
		debug_log("Redrew line %p, lineno=%zu, row=%d",
	...
	if (openfile->mark) {
		...
		while (mark_line != openfile->current && mark_line != NULL) {
			...
			debug_log("Updated marked line %p, lineno=%zu",
			...
			if (lines_updated > 1000) {
				debug_log("Aborting mark update, too many iterations");
				break;
			}
		}
	}
	...
	if (old_current != openfile->current && old_current != NULL && get_page_start(was_pww) > 0) {
		...
		debug_log("Updated old_current %p, lineno=%zu",
	...
	if (line_needs_update(was_pww, openfile->placewewant) ||
		...
		update_line(openfile->current, openfile->current_x);
		debug_log("Updated current %p, lineno=%zu, x=%zu",
	...
	debug_log("Completed, popup_active=%d, current_y=%d, current_x=%zu",
...
#include <time.h>
	...
	if (footwin) {
		...
		wnoutrefresh(footwin);
		wattroff(footwin, A_BOLD);
		debug_log("footwin refreshed with color_pair=1");
	}
	if (doupdate() == ERR) {
