...
extern int get_current_y(const struct openfilestruct *file);
...
#ifdef ENABLE_YCMD
/* Modified and portions code generated by Grok. */
int get_kbinput(WINDOW *frame, bool showcursor) {
    ...
    if (frame == get_popup()) {
        nodelay(frame, TRUE);
        timeout(50);
        curs_set(0); // Hide cursor in popup
    } else {
        timeout(-1);
        curs_set(1);
    }
    ...
    while (kbinput == ERR) {
        ...
        if (kbinput == ERR && frame == get_popup()) {
            int ch = wgetch(frame);
            if (ch == 0x1B) {
                kbinput = 0x1B;
                snprintf(msg, sizeof(msg), "get_kbinput: Detected raw ESC (0x1B), frame=%p\n",
                         (void *)frame);
                fprintf(stderr, msg);
                timeout(20);
                int trailing;
                while ((trailing = wgetch(frame)) != ERR) {
                    snprintf(msg, sizeof(msg), "get_kbinput: Consumed trailing byte 0x%x\n", trailing);
                    fprintf(stderr, msg);
                }
                timeout(50);
                break;
            } else if (ch == KEY_RESIZE) {
                snprintf(msg, sizeof(msg), "get_kbinput: Terminal resized, closing popup\n");
                fprintf(stderr, msg);
                hide_completions();
                ycmd_handling = FALSE;
                is_popup_mode = FALSE;
                ...
                ungetch(ch);
                snprintf(msg, sizeof(msg), "get_kbinput: Pushed back ch=0x%x\n", ch);
                fprintf(stderr, msg);
            }
        }
        clock_gettime(CLOCK_MONOTONIC, &now);
        double elapsed = (now.tv_sec - start.tv_sec) + (now.tv_nsec - start.tv_nsec) / 1e9;
        if (elapsed > 5.0 && frame == get_popup()) {
            snprintf(msg, sizeof(msg), "get_kbinput: Popup timeout after %.2fs, closing popup\n", elapsed);
            fprintf(stderr, msg);
            hide_completions();
            ycmd_handling = FALSE;
            is_popup_mode = FALSE;
    ...
    if (frame == get_popup()) {
        nodelay(frame, FALSE);
        timeout(-1);
        // Ensure popup remains focused
        if (kbinput == KEY_UP || kbinput == KEY_DOWN) {
            snprintf(msg, sizeof(msg), "get_kbinput: Popup navigation input=0x%x\n", kbinput);
            fprintf(stderr, "%s\n", msg);
        }
    }
    ...
    snprintf(msg, sizeof(msg), "get_kbinput: returning 0x%x\n", kbinput);
    fprintf(stderr, "%s\n", msg);
    return kbinput;
...
void edit_redraw(linestruct *old_current, update_type manner) {
    char msg[256];
    snprintf(msg, sizeof(msg), "edit_redraw: old_current=%p, current=%p, filetop=%p, manner=%d\n",
             (void *)old_current, (void *)openfile->current, (void *)openfile->filetop, manner);
    fprintf(stderr, msg);
    ...
    if (current_is_offscreen()) {
        ...
        refresh_needed = TRUE;
        snprintf(msg, sizeof(msg), "edit_redraw: Current offscreen, setting refresh_needed\n");
        fprintf(stderr, msg);
        return;
    }

    if (openfile->filetop == NULL || openfile->current == NULL) {
        snprintf(msg, sizeof(msg), "edit_redraw: Invalid linestruct, filetop=%p, current=%p\n",
                 (void *)openfile->filetop, (void *)openfile->current);
        fprintf(stderr, msg);
        return;
    }

    if (!is_popup_active()) {
        wclear(midwin); /* Clear only if popup is inactive */
        snprintf(msg, sizeof(msg), "edit_redraw: Cleared midwin (popup inactive)\n");
        fprintf(stderr, msg);
    }

    linestruct *line = openfile->edittop;
    int row = 0;
    int maxrows = editwinrows > 0 ? editwinrows : getmaxy(midwin) - (ISSET(ZERO) ? 0 : 1);
    while (line != NULL && row < maxrows) {
        update_line(line, line == openfile->current ? openfile->current_x : 0);
        snprintf(msg, sizeof(msg), "edit_redraw: Redrew line %p, lineno=%zu, row=%d\n",
                 (void *)line, line->lineno, row);
        fprintf(stderr, msg);
        line = line->next;
        row++;
    }
...
#ifndef NANO_TINY
    if (openfile->mark) {
        linestruct *mark_line = openfile->mark;
        int lines_updated = 0;
        while (mark_line != openfile->current && mark_line != NULL) {
            update_line(mark_line, 0);
            snprintf(msg, sizeof(msg), "edit_redraw: Updated marked line %p, lineno=%zu\n",
                     (void *)mark_line, mark_line->lineno);
            fprintf(stderr, msg);
            mark_line = (mark_line->lineno > openfile->current->lineno) ? mark_line->prev : mark_line->next;
            lines_updated++;
            if (lines_updated > 1000) {
                snprintf(msg, sizeof(msg), "edit_redraw: Aborting mark update, too many iterations\n");
                fprintf(stderr, msg);
                break;
            }
        }
    ...
    if (old_current != openfile->current && old_current != NULL && get_page_start(was_pww) > 0) {
        update_line(old_current, 0);
        snprintf(msg, sizeof(msg), "edit_redraw: Updated old_current %p, lineno=%zu\n",
                 (void *)old_current, old_current->lineno);
        fprintf(stderr, msg);
    }
    ...
    if (line_needs_update(was_pww, openfile->placewewant) ||
        ...
        update_line(openfile->current, openfile->current_x);
        snprintf(msg, sizeof(msg), "edit_redraw: Updated current %p, lineno=%zu, x=%zu\n",
                 (void *)openfile->current, openfile->current->lineno, openfile->current_x);
        fprintf(stderr, msg);
    }
    ...
    if (is_popup_active()) {
        ...
        snprintf(msg, sizeof(msg), "edit_redraw: Refreshed popup\n");
        fprintf(stderr, msg);
    }
    doupdate();

    snprintf(msg, sizeof(msg), "edit_redraw: Completed, popup_active=%d, current_y=%d, current_x=%zu\n",
             is_popup_active(), get_current_y(openfile), openfile->current_x);
    fprintf(stderr, msg);
}
#else
...
void full_refresh(void) {
    char msg[256];
    if (topwin) wnoutrefresh(topwin);
    if (midwin) {
        wmove(midwin, get_current_y(openfile), openfile->current_x);
        curs_set(is_popup_active() ? 0 : 1);
        wnoutrefresh(midwin);
    }
    ...
    if (doupdate() == ERR) {
        snprintf(msg, sizeof(msg), "full_refresh: doupdate failed\n");
        fprintf(stderr, "%s\n", msg);
    }
}
#else
