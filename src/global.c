...
#ifdef ENABLE_YCMD
	add_to_funcs(do_cancel, (((MMOST|MREFACTORRENAME) & ~MMAIN) | MYESNO),
#else
...
#define SLASH_OR_DASH  "^/"
...
#ifdef ENABLE_YCMD
		add_to_sclist((MMOST|MBROWSER) & ~MFINDINHELP & ~MYCMEXTRACONF & ~MCOMPLETERCOMMANDS & ~MCODECOMPLETION, help_key, 0, do_help, 0);
#else
	...
	{
#ifdef ENABLE_YCMD
		add_to_sclist(MMOST|MBROWSER|MHELP|MCODECOMPLETION, "Left", KEY_LEFT, do_left, 0);
		add_to_sclist(MMOST|MBROWSER|MHELP|MCODECOMPLETION, "Right", KEY_RIGHT, do_right, 0);
#else
...
#ifdef ENABLE_YCMD
	add_to_sclist(MMOST|MCODECOMPLETION, "Home", KEY_HOME, do_home, 0);
#else
	add_to_sclist(MMOST, "Home", KEY_HOME, do_home, 0);
#endif
#ifdef ENABLE_YCMD
	add_to_sclist(MMOST|MCODECOMPLETION, "End", KEY_END, do_end, 0);
#else
