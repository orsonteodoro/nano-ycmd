...
void version(void)
{
	...
	printf(_("\n License:  LGPL-2.1+"));
	printf(_("\n Web:  https://www.gnu.org/software/libc/"));
	printf(_("\n"));
...
void unbound_key(int code)
{
	...
	if (code == 0x1B && is_popup_active()) {
		debug_log("Skipping ESC (0x1B), popup_active=%d", is_popup_active());
		return;
	}
#endif

	if (code < 0x20) {
		debug_log("Processing code=0x%x", code);
...
int get_current_y(const openfilestruct *file) {
	...
	debug_log("y=%d, current=%p, edittop=%p",
...
void process_a_keystroke_popup(void)
{
	...
	debug_log("Before get_kbinput, popup_active=%d, ycmd_handling=%d, is_popup_mode=%d",
	...
	debug_log("input=0x%x (%c), popup_active=%d, frame=%p, is_popup_mode=%d",
	...
	if (is_popup_active()) {
		if (input == ERR) {
			debug_log("Ignoring ERR input for popup");
			return;
		}
		...
		int length_to_delete = original_x - word_start;
			...
			full_refresh();
			debug_log("Handled navigation input=0x%x", input);
			return;
		}
	...
	if (just_closed_popup && input == 0x0A) {
		debug_log("Skipped Enter (0x0A) after popup closure");
	...
	if (input == 0x00) { /* Ctrl+Space */
		debug_log("Ctrl+Space start, is_popup_mode=%d", is_popup_mode);
	...
	json_t *completions = request_completions(
		...
		debug_log("Ctrl+Space end, is_popup_mode=%d", is_popup_mode);
		return;
	}
...
#define NOTREBOUND  TRUE
	...
	while (TRUE) {
#ifdef ENABLE_YCMD
		debug_log("Before checks, popup_active=%d, ycmd_handling=%d, refresh_needed=%d",
...
#ifdef ENABLE_YCMD
		process_a_keystroke();
		debug_log("After process_a_keystroke, popup_active=%d, ycmd_handling=%d",
